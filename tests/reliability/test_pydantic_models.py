...
def test_qa_with_pydantic_answer_model():
    ...
    assert_program_output_correct(
        ...
        grading_guidelines=("The comments should be relevant to the answer. They don't need to restate the answer explicitly."
                            ),
    )
...
def test_entity_extraction_with_multiple_primitive_outputs():
    ...
    class ExtractEntityFromDescriptionOutput(pydantic.BaseModel):
        entity_hu: str = pydantic.Field(
            description="The extracted entity in Hungarian, cleaned and lowercased.")
        entity_en: str = pydantic.Field(
        ...
        categories: str = pydantic.Field(
            description="English categories separated by '|' to which the entity belongs.")
    ...
    class ExtractEntityFromDescription(dspy.Signature):
        ...
        description: str = dspy.InputField(
            description="The input description in Hungarian.")
    ...
    assert_program_output_correct(
        ...
        grading_guidelines=("The extracted entity should be associated with English language categories that apply to the word 'coffee'."
...
def test_tool_calling_with_literals(module):
    ...
    class ToolCalling(dspy.Signature):
        ...
        response_status: Literal["success", "error",
                                 "pending"] = dspy.OutputField()
        user_intent: Literal["informational", "transactional",
                             "exploratory"] = dspy.OutputField()

    program = dspy.Predict(ToolCalling)
    prediction = program(
        question=("Tell me more about the company's internal policy for paid time off (PTO), "
