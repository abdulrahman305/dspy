...
async def test_convert_mcp_tool():
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            assert hello_tool.arg_desc == {
                "names": "No description provided. (Required)"
            }
            ...
            current_datetime_tool = convert_mcp_tool(session,
                                                     response.tools[4])
            ...
            assert await current_datetime_tool.acall(
            ) == "2025-07-23T09:10:10.0+00:00"
