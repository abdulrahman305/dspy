...
def test_save_and_load_with_json(tmp_path):
    ...
    model.predict.demos = [
        dspy.Example(q="What is the capital of France?",
                     a="Paris", reasoning="n/a").with_inputs("q"),
        # Nested example
        dspy.Example(
            q=[
                dspy.Example(q="What is the capital of France?"),
...
def test_save_and_load_with_pkl(tmp_path):
    ...
    trainset = [dspy.Example(
        **example).with_inputs("current_date", "target_date") for example in trainset]
    ...
    optimizer = dspy.BootstrapFewShot(
        max_bootstrapped_demos=4, max_labeled_demos=4, max_rounds=5, metric=dummy_metric)
...
def test_single_module_call_with_usage_tracker(lm_for_test):
    dspy.settings.configure(lm=dspy.LM(
        lm_for_test, cache=False), track_usage=True)
    ...
    dspy.settings.configure(lm=dspy.LM(
        lm_for_test, cache=True), track_usage=True)
    for _ in range(2):
        output = predict(question="What is the capital of France?")
...
def test_module_history():
    ...
    with patch("litellm.completion") as mock_completion:
        ...
        dspy.settings.configure(lm=dspy.LM(
            "openai/gpt-4o-mini", cache=False), adapter=dspy.JSONAdapter())
        program = MyProgram()
