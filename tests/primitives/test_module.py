import dspy
from dspy.primitives.module import (  # Adjust the import based on your file structure
    Module, set_attribute_by_name,
)
...
def test_named_predictors():
    ...
    assert len(named_preds
               ) == 2, "Should identify correct number of Predict instances"
...
def test_predictors():
    ...
    assert all(isinstance(p, dspy.Predict) for p in
               preds), "All returned items should be instances of PredictMock"
...
def test_complex_module_set_attribute_by_name():
    ...
    set_attribute_by_name(root, "sub_module.nested_list[1]['key'].test_attrib",
                          True)
    ...
    set_attribute_by_name(root, "sub_module.nested_tuple[1][0].test_attrib",
                          True)
