...
def test_batch_module():
    ...
    result, reason_result = MyModule()(
        dspy.Example(input="test input").with_inputs("input"))
...
def test_nested_parallel_module():
    ...
    assert {output[0].output, output[1].output} <= {
        f"test output {i}" for i in range(1, 5)}
    assert {output[2][0].output, output[2][1].output} <= {
        f"test output {i}" for i in range(1, 5)}
    all_outputs = {
        output[0].output,
        output[1].output,
...
def test_nested_batch_method():
    ...
    class MyModule(dspy.Module):
        ...
        def forward(self, input):
            res = self.predictor.batch(
                [dspy.Example(input=input).with_inputs("input")] * 2)

            return res

    result = MyModule().batch(
        [dspy.Example(input="test input").with_inputs("input")] * 2)
