...
def test_baml_adapter_formats_pydantic_inputs_as_clean_json():
    ...
    messages = adapter.format(
        TestSignature, [], {"patient": patient, "question": "What is the diagnosis?"})
...
def test_baml_adapter_handles_mixed_input_types():
    ...
    messages = adapter.format(
        TestSignature, [], {"patient": patient, "priority": 1, "notes": "Urgent case"})
...
def test_baml_adapter_field_alias_without_description():
    ...
    regular_field_section = schema.split("regular_field: int,")[
        0].split("\n")[-1]
...
def test_baml_adapter_multiple_pydantic_input_fields():
    ...
    system_config = SystemConfig(
        timeout=300, debug=True, endpoints=["api1", "api2"])

    messages = adapter.format(
        TestSignature, [], {"input_1": user_profile, "input_2": system_config})
