...
def test_chat_adapter_quotes_literals_as_expected(input_literal,
                                                  output_literal, input_value,
                                                  expected_input_str,
                                                  expected_output_str):
    """
    ...
    with mock.patch("litellm.completion") as mock_completion:
        program(
            owner=PetOwner(
                name="John",
                num_pets=5,
                dogs=DogClass(dog_breeds=["labrador", "chihuahua"],
                              num_dogs=2),
            ),
...
def test_chat_adapter_fallback_to_json_adapter_on_exception():
    ...
    with mock.patch("litellm.completion") as mock_completion:
        ...
        with mock.patch("dspy.adapters.json_adapter.JSONAdapter.__call__"
...
async def test_chat_adapter_fallback_to_json_adapter_on_exception_async():
    ...
    with mock.patch("litellm.acompletion") as mock_completion:
        ...
        with mock.patch("dspy.adapters.json_adapter.JSONAdapter.acall"
