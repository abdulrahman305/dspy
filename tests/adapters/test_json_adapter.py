    ...
    def clean_schema_extra(field_name, field_info):
        ...
        if "json_schema_extra" in attrs:
            attrs["json_schema_extra"] = {
                ...
                if k != "__dspy_field_type"
                and not (k == "desc" and v == f"${{{field_name}}}")
            }
        return attrs
...
def test_json_adapter_falls_back_when_structured_outputs_fails():
    ...
    with mock.patch("litellm.completion") as mock_completion:
        ...
        assert issubclass(first_call_kwargs.get("response_format"),
                          pydantic.BaseModel)
...
async def test_json_adapter_async_call():
    ...
    result = await adapter.acall(
        lm, {}, signature, [], {"question": "What is the capital of France?"})
    assert result == [{"answer": "Paris"}]
...
def test_json_adapter_formats_image_with_nested_images():
    ...
    image_wrapper = ImageWrapper(images=[image1, image2, image3],
                                 tag=["test", "example"])
...
def test_json_adapter_with_tool():
    ...
    adapter = dspy.JSONAdapter()
    messages = adapter.format(MySignature, [], {
...
def test_json_adapter_with_code():
    ...
    adapter = dspy.JSONAdapter()
    messages = adapter.format(CodeAnalysis, [],
...
async def test_json_adapter_on_pydantic_model_async():
    ...
    with mock.patch("litellm.acompletion") as mock_completion:
        ...
        with dspy.context(lm=dspy.LM(model="openai/gpt4o", cache=False),
                          adapter=dspy.JSONAdapter()):
...
def test_json_adapter_fallback_to_json_mode_on_structured_output_failure():
    ...
    dspy.configure(lm=dspy.LM(model="openai/gpt-4o-mini", cache=False),
                   adapter=dspy.JSONAdapter())
    program = dspy.Predict(TestSignature)
    ...
    with mock.patch("litellm.completion") as mock_completion:
        ...
        mock_completion.side_effect = [
            ...
            ModelResponse(choices=[
                Choices(message=Message(content="{'answer': 'Test output'}"))
            ]),
        ]
...
def test_json_adapter_toolcalls_native_function_calling():
    ...
    with mock.patch("litellm.completion") as mock_completion:
            ...
            choices=[
                Choices(
                    ...
                    message=Message(
                        ...
                        tool_calls=[
                            ChatCompletionMessageToolCall(
                                function=Function(arguments='{"city":"Paris"}',
                                                  name="get_weather"),
        ...
        assert result[0]["tool_calls"] == dspy.ToolCalls(tool_calls=[
