...
def test_lm_calls_are_cached_across_lm_instances(litellm_test_server,
                                                 temporary_blank_cache_dir):
...
def test_lm_calls_are_cached_in_memory_when_expected(
        litellm_test_server, temporary_blank_cache_dir):
    api_base, server_log_file_path = litellm_test_server
...
def test_lms_called_expected_number_of_times_for_cache_key_generation_failures(
    ...
    with pytest.raises(RuntimeError), patch(
            "litellm.completion") as mock_completion:
