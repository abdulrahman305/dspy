...
def format_field_value(field_info: FieldInfo,
                       value: Any,
                       assume_text=True) -> str | dict:
    """
    ...
    if isinstance(value, list) and field_info.annotation is str:
        ...
        if isinstance(jsonable_value, dict) or isinstance(
                jsonable_value, list):
...
def translate_field_type(field_name, field_info):
    ...
    if get_dspy_field_type(field_info) == "input" or field_type is str:
        ...
        desc = f"must be one of: {enum_vals}"
                 ...
                 "__origin__") and field_type.__origin__ is Literal:
        desc = (
...
def parse_value(value, annotation):
    ...
    candidate = json_repair.loads(
        value)  # json_repair.loads returns "" on failure.
    ...
    try:
        ...
        if origin is Union and type(None) in get_args(
                annotation) and str in get_args(annotation):
...
def get_annotation_name(annotation):
    ...
    if origin is Literal:
        args_str = ", ".join(
            (_quoted_string_for_literal_type_annotation(a) if isinstance(
                a, str) else get_annotation_name(a)) for a in args)
...
def get_field_description_string(fields: dict) -> str:
    ...
    for idx, (k, v) in enumerate(fields.items()):
        ...
        field_message += (
            ...
            if v.json_schema_extra.get("constraints") else "")
