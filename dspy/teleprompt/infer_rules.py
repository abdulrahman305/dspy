...
class InferRules(BootstrapFewShot):
    ...
    def compile(self, student, *, teacher=None, trainset, valset=None):
        ...
        all_predictors = [
            p for p in original_program.predictors()
        ...
        for candidate_idx in range(self.num_candidates):
            ...
            candidate_predictors = [
                p for p in candidate_program.predictors()
                if hasattr(p, "signature")
            ]
    ...
    def format_examples(self, demos, signature):
        ...
        for demo in demos:
            input_fields = {
                k: v
                for k, v in demo.items() if k in signature.input_fields
            }
            output_fields = {
                k: v
                for k, v in demo.items() if k in signature.output_fields
            }
            input_text = "\n".join(f"{k}: {v}"
