...
class GRPO(FinetuneTeleprompter):
    def __init__(
        ...
        variably_invoked_predictor_grouping_mode: (
            Literal["truncate"] | Literal["fill"] | Literal["ragged"]
        ) = "truncate",
        variably_invoked_predictor_fill_strategy: (
            Literal["randint"] | Literal["max"] | None) = None,
    ):
    ...
    def report_validation_metrics(self, student, trainset, valset, logger, step_idx=-1):
        ...
        if valset is not None:
            ...
            if self.report_train_scores:
                ...
                valset_evaluation = valset_evaluator(
                    student, metric=self.metric)
                trainset_scores = [r[-1]
                                   for r in valset_evaluation.results[len(valset):]]
                valset_scores = [r[-1]
                                 for r in valset_evaluation.results[: len(valset)]]
    ...
    def compile(
        ...
        **kwargs,
        ...
        assert len(
            trainset) > 0, "Training set is empty. Please provide a non-empty training set."
        ...
        for train_step_idx in range(self.num_train_steps):
            ...
            def _any_available_for_step():
                for _, job in grpo_training_jobs.items():
                    ...
                    available = set(pending_batch_ids) - \
                        set(self.fulfilled_batch_ids)
                    if available:
                        return True
                return False
            ...
            trace_data = [[[] for _ in range(len(teachers))] for _ in range(
                len(subsample_training_dataset))]
            for tind, teacher in enumerate(teachers):
                ...
                round_data = bootstrap_trace_data(
                    ...
                    metric=self.metric,
                    num_threads=self.num_threads,
                    # TODO(GRPO Team): This should be True, once the dspy format issue is fixed
            ...
            for (lm_for_job, data_key), job in grpo_training_jobs.items():
                train_data: list[GRPOGroup] = (
                    sum(train_batch_per_predictor, [
                        ]) if data_key is None else train_batch_per_predictor[data_key]
                )  # noqa: RUF017
                ...
                if len(q) < len(available_batch_ids) and len(train_data) > 0:
                    ...
                    while need > 0:
                        ...
                        shuffled = self.rng.sample(
                            train_data, k=len(train_data))
                        q.extend(shuffled)
                        need -= len(shuffled)
                ...
                for bid in available_batch_ids:
                    if q:
                        ...
                        fallback_pool = train_data if len(
                            train_data) > 0 else sum(train_batch_per_predictor, [])
