...
class GRPO(FinetuneTeleprompter):
    ...
    def validate_trace_data_and_log_issues(
        ...
        if len(trace_data[0][0]) == 0:
            ...
            assert "trace" in trace_data[0][0][
                0], "Trace data does not contain the 'trace' key"
            ...
            for teacher_data in example_data:
                for sample in teacher_data:
                    for t in sample["trace"]:
                        assert hash(
                            t[0].signature) in pred_signature_hash_to_ind
    ...
    def report_validation_metrics(self,
                                  ...
                                  logger,
                                  step_idx=-1):
        if step_idx == -1 or step_idx == self.num_train_steps - 1 or (
                step_idx + 1) % self.num_steps_for_val == 0:
            pass
        else:
            return
        ...
        if valset is not None:
            ...
            assert isinstance(
                self.num_steps_for_val, int) and self.num_steps_for_val > 0, (
            ...
            if self.report_train_scores:
                ...
                valset_evaluation = valset_evaluator(student,
                                                     metric=self.metric)
                trainset_scores = [
            ...
            if self.report_train_scores:
                ...
                assert isinstance(
                    self.num_steps_for_val,
                    int) and self.num_steps_for_val > 0, (
                        "num_steps_for_val must be a positive integer.")
                if step_idx == -1:
    ...
    def select_training_sample_and_update_shuffled_trainset(
        ...
        if curr_epoch > self.epoch:
            logger.info(
                f"Updating shuffled trainset for epoch {curr_epoch}...")
            self.epoch = curr_epoch
            self.update_shuffled_trainset(original_trainset)

        assert len(
            self.shuffled_trainset_ids
        ) >= self.num_dspy_examples_per_grpo_step, (
        ...
        assert len(
            self.shuffled_trainset_ids
        ) % self.num_dspy_examples_per_grpo_step == 0, (
    ...
    def compile(
        ...
        **kwargs,
        ...
        assert len(
            trainset
        ) > 0, "Training set is empty. Please provide a non-empty training set."
        ...
        if len(trainset) < self.num_dspy_examples_per_grpo_step:
            ...
            multiplier = (self.num_dspy_examples_per_grpo_step +
                          len(trainset) - 1) // len(trainset)
            if multiplier > 1:
                logger.warning(
        ...
        pred_signature_hash_to_ind = {
            hash(pred.signature): ind
            for ind, pred in enumerate(student.predictors())
        }
        ...
        if (isinstance(teacher, list)
                and len(teacher) == 0) or teacher is None:
            teacher = student
        ...
        num_samples_per_input = self.num_rollouts_per_grpo_step // len(
            teachers)
        ...
        for train_step_idx in range(self.num_train_steps):
            ...
            for tind, teacher in enumerate(teachers):
                ...
                round_data = bootstrap_trace_data(
                    ...
                    metric=self.metric,
                    num_threads=self.num_threads,
                    raise_on_error=# TODO(GRPO Team): This should be True, once the dspy format issue is fixed
                    False,
                ...
                for data_dict in round_data:
                    ...
                    trace_data[example_ind_in_subsample][tind].append(
                        data_dict)
            ...
            for pred_id in range(num_student_predictors):
                for example_ind, example_data in enumerate(trace_data):
                    ...
                    if len(predictor_example_invocations) == 0:
                        ...
                        continue
                    elif len(predictor_example_invocations
                    ...
                    if self.variably_invoked_predictor_grouping_mode == "truncate":
                        ...
                        if self.variably_invoked_predictor_fill_strategy == "randint":
                            ...
                            def selector(l):
                                return self.rng.choice(l)  # noqa: E731, E741

                        else:

                            def selector(l):
                                return l[-1]  # noqa: E731, E741
                    ...
                    for group_idx in range(max_len):
                        for rollout_idx in range(
                            ...
                            trace_instance = predictor_example_invocations[
                                rollout_idx][group_idx]
                            ...
                            adapter = self.adapter[
                                pred_lm] or settings.adapter or ChatAdapter()
                            ...
                            if isinstance(trace_instance[2], FailedPrediction):
                                score = trace_instance[
                                    2].format_reward or self.format_failure_score
                                example_training_data[group_idx].append({
                                    ...
                                    "completion": {
                                        ...
                                        "assistant",
                                        "content":
                                        trace_instance[2].completion_text,
                                    },
                                    "reward":
                                    float(score),
                                })
                    ...
                    train_batch_per_predictor[pred_id].extend(
            ...
            for (_, data_key), job in grpo_training_jobs.items():
                train_data: list[GRPOGroup] = (
                    sum(train_batch_per_predictor, []) if data_key is None else
                    train_batch_per_predictor[data_key])  # noqa: RUF017
                ...
                job.step(train_data=train_data,
        ...
        logger.info(
            "Done with the iterations! Retrieving the final model(s)...")
