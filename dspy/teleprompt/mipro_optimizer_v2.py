...
class MIPROv2(Teleprompter):
    ...
    def compile(
        ...
        *,
        ...
        zeroshot_opt = (self.max_bootstrapped_demos
                        == 0) and (self.max_labeled_demos == 0)
        ...
        if self.auto is None and (self.num_candidates is not None
                                  and num_trials is None):
            raise ValueError(
        ...
        if self.auto is None and (self.num_candidates is None
                                  or num_trials is None):
            raise ValueError(
        ...
        if self.auto is not None and (self.num_candidates is not None
                                      or num_trials is not None):
            raise ValueError(
    ...
    def _set_num_trials_from_num_candidates(self, program, zeroshot_opt,
                                            num_candidates):
    ...
    def _set_hyperparams_from_run_mode(
        ...
        valset = create_minibatch(valset,
                                  batch_size=auto_settings["val_size"],
                                  rng=self.rng)
        ...
        self.num_instruct_candidates = auto_settings[
            "n"] if zeroshot_opt else int(auto_settings["n"] * 0.5)
    ...
    def _set_and_validate_datasets(self, trainset: list, valset: list | None):
        ...
        if valset is None:
            ...
            if len(valset) < 1:
                raise ValueError(
                    "Validation set must have at least 1 example.")

        return trainset, valset

    def _print_auto_run_settings(self, num_trials: int, minibatch: bool,
                                 valset: list):
        logger.info(
    ...
    def _estimate_lm_calls(
        ...
        estimated_prompt_model_calls = (
            ...
            num_predictors  # Candidate generation
        ...
        if not minibatch:
            ...
            estimated_task_model_calls = minibatch_size * num_trials + len(
                valset) * full_eval_steps
            task_model_line = (
    ...
    def _bootstrap_fewshot_examples(self, program: Any, trainset: list,
                                    seed: int, teacher: Any) -> list | None:
        ...
        demo_candidates = create_n_fewshot_demo_sets(
            ...
            trainset=trainset,
            max_labeled_demos=(LABELED_FEWSHOT_EXAMPLES_IN_CONTEXT
                               if zeroshot else self.max_labeled_demos),
            max_bootstrapped_demos=(BOOTSTRAPPED_FEWSHOT_EXAMPLES_IN_CONTEXT if
                                    zeroshot else self.max_bootstrapped_demos),
            metric=self.metric,
            max_errors=effective_max_errors,
            teacher=teacher,
    ...
    def _optimize_prompt_parameters(
        ...
        adjusted_num_trials = int((
            ...
            run_additional_full_eval_at_end) if minibatch else num_trials)
        logger.info(
            f"== Trial {1} / {adjusted_num_trials} - Full Evaluation of Default Program =="
        )

        default_score = eval_candidate_program(len(valset), valset, program,
                                               evaluate, self.rng).score
        ...
        def objective(trial):
            ...
            if not minibatch and score > best_score:
                ...
                logger.info(
                    f"{GREEN}Best full score so far!{ENDC} Score: {score}")
            ...
            if minibatch and ((trial_num %
                               (minibatch_full_eval_steps + 1) == 0) or
        ...
        default_params = {
            f"{i}_predictor_instruction": 0
            for i in range(len(program.predictors()))
        }
        if demo_candidates:
            default_params.update({
                f"{i}_predictor_demos": 0
                for i in range(len(program.predictors()))
            })
        ...
        if best_program is not None and self.track_stats:
            ...
            sorted_candidate_programs = sorted(score_data,
                                               key=lambda x: x["score"],
                                               reverse=True)
            # Attach all minibatch programs
            best_program.mb_candidate_programs = [
                score_data for score_data in sorted_candidate_programs
        ...
        logger.info(
            f"Returning best identified program with score {best_score}!")
    ...
    def _log_normal_eval(
        ...
        trial_logs[trial_num]["total_eval_calls_so_far"] = total_eval_calls
        trial_logs[trial_num][
            "full_eval_program"] = candidate_program.deepcopy()
    ...
    def _select_and_insert_instructions_and_demos(
        ...
        for i, predictor in enumerate(candidate_program.predictors()):
            ...
            chosen_params.append(
                f"Predictor {i}: Instruction {instruction_idx}")
            ...
            if demo_candidates:
                ...
                chosen_params.append(
                    f"Predictor {i}: Few-Shot Set {demos_idx}")
                raw_chosen_params[f"{i}_predictor_demos"] = instruction_idx

        return chosen_params, raw_chosen_params

    def _get_param_distributions(self, program, instruction_candidates,
                                 demo_candidates):
        ...
        for i in range(len(instruction_candidates)):
            param_distributions[
                f"{i}_predictor_instruction"] = CategoricalDistribution(
                    range(len(instruction_candidates[i])))
            if demo_candidates:
                param_distributions[
                    f"{i}_predictor_demos"] = CategoricalDistribution(
    ...
    def _perform_full_evaluation(
        ...
        trial_logs[trial_num +
                   1]["full_eval_program_path"] = save_candidate_program(
                       program=highest_mean_program,
