...
class MIPROv2(Teleprompter):
    ...
    def _set_hyperparams_from_run_mode(
        ...
        self.num_instruct_candidates = auto_settings["n"] if zeroshot_opt else int(
            auto_settings["n"] * 0.5)
    ...
    def _estimate_lm_calls(
        ...
        if not minibatch:
            ...
            estimated_task_model_calls = minibatch_size * \
                num_trials + len(valset) * full_eval_steps
            task_model_line = (
    ...
    def _bootstrap_fewshot_examples(self, program: Any, trainset: list, seed: int, teacher: Any) -> list | None:
        ...
        demo_candidates = create_n_fewshot_demo_sets(
            ...
            trainset=trainset,
            max_labeled_demos=(
                LABELED_FEWSHOT_EXAMPLES_IN_CONTEXT if zeroshot else self.max_labeled_demos),
    ...
    def _optimize_prompt_parameters(
        ...
        adjusted_num_trials = int(
            (num_trials + num_trials // minibatch_full_eval_steps +
             1 + run_additional_full_eval_at_end)
            if minibatch
            else num_trials
        )
        logger.info(
        ...
        default_score = eval_candidate_program(
            len(valset), valset, program, evaluate, self.rng).score
        ...
        score_data = [{"score": best_score,
                       "program": program.deepcopy(), "full_eval": True}]
        ...
        def objective(trial):
            ...
            score = eval_candidate_program(
                batch_size, valset, candidate_program, evaluate, self.rng).score
            ...
            if minibatch and (
                (trial_num % (minibatch_full_eval_steps + 1) ==
                 0) or (trial_num == (adjusted_num_trials - 1))
            ):
        ...
        default_params = {f"{i}_predictor_instruction": 0 for i in range(
    ...
    def _select_and_insert_instructions_and_demos(
        ...
        for i, predictor in enumerate(candidate_program.predictors()):
            ...
            instruction_idx = trial.suggest_categorical(
                f"{i}_predictor_instruction", range(
                    len(instruction_candidates[i]))
            )
            selected_instruction = instruction_candidates[i][instruction_idx]
            updated_signature = get_signature(
                predictor).with_instructions(selected_instruction)
    ...
    def _perform_full_evaluation(
        ...
        full_eval_score = eval_candidate_program(
            len(valset), valset, highest_mean_program, evaluate, self.rng).score
        score_data.append(
            {
