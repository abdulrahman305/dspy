...
class SIMBA(Teleprompter):
    ...
    def __init__(
        ...
        *,
        ...
        if self.max_demos > 0:
            self.strategies = [
                append_a_demo(demo_input_field_maxlen), append_a_rule
            ]
        else:
    ...
    def compile(self,
        ...
        assert len(
            trainset
        ) >= self.bsize, f"Trainset too small: {len(trainset)} < {self.bsize}"
        ...
        def top_k_plus_baseline(k: int) -> list[int]:
            ...
            scored_programs = sorted(
                programs,
                key=lambda p: calc_average_score(p.simba_idx),
                reverse=True)
        ...
        def softmax_sample(rng_obj: random.Random, program_idxs: list[int],
        ...
        # Parallel runner
        run_parallel = dspy.Parallel(access_examples=False,
        ...
        for batch_idx in range(self.max_steps):
            ...
            batch_indices = data_indices[instance_idx:instance_idx +
                                         self.bsize]
            ...
            models = prepare_models_for_resampling(programs[0],
                                                   self.num_candidates)
            ...
            for bucket_idx, (bucket, bucket_stats) in enumerate(buckets):
                ...
                for _, predictor in name2predictor.items():
                    predictor.demos = [
                        demo for idxd, demo in enumerate(predictor.demos)
            ...
            outputs = run_parallel(exec_pairs)
            assert len(outputs) == len(
                exec_pairs) == len(system_candidates) * self.bsize
        ...
        candidate_programs = [
            winning_programs[i].deepcopy() for i in program_idxs
        ]
        logger.info(
        ...
        for idx_prog, _ in enumerate(candidate_programs):
            ...
            sys_scores = [outputs[i]["score"] for i in range(start, end)]
            avg_score = sum(sys_scores) / len(
                sys_scores) if sys_scores else 0.0
