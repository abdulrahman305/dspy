...
class AvatarOptimizer(Teleprompter):
    ...
    def thread_safe_evaluator(self,
                              devset,
                              actor,
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(self.process_example, actor, example,
                                return_outputs) for example in devset
            ]
    ...
    def _get_pos_neg_results(
        ...
        avg_score, results = self.thread_safe_evaluator(trainset,
                                                        actor,
                                                        return_outputs=True)
    ...
    def compile(self, student, *, trainset):
        ...
        for i in range(self.max_iters):
            ...
            if self.max_positive_inputs and len(
                    pos_inputs) > self.max_positive_inputs:
                pos_inputs = sample(pos_inputs, self.max_positive_inputs)

            if self.max_negative_inputs and len(
                    neg_inputs) > self.max_negative_inputs:
            ...
            if (self.optimize_for == "max"
                    and best_score < score) or (self.optimize_for == "min"
                                                and best_score > score):
                best_actor.actor.signature = best_actor.actor.signature.with_instructions(
                    new_instruction)
