...
def prepare_models_for_resampling(program: dspy.Module, n: int, teacher_settings: dict | None = None):
    ...
    models.extend([lm.copy(rollout_id=r, temperature=1.0)
                  for r in rollout_ids[start_rollout_idx:]])
        ...
        try:
            ...
            if isinstance(output, (int, float)):
                ...
                if not hasattr(output, "score"):
                    raise ValueError(
                        "When `metric` returns a `dspy.Prediction`, it must contain a `score` field.")
                score = output.score
                # Extract fields from the output dspy.Prediction, excluding `score``
                output_metadata = {k: v for k,
                                   v in output.items() if k != "score"}
        except Exception as e:
            logger.warning(e)
...
def append_a_demo(demo_input_field_maxlen):
    def append_a_demo_(bucket, system, **kwargs):
        ...
        if good["score"] <= batch_10p_score:
            logger.info(
                f"Skipping appending a demo as good score {good['score']} is at or below the 10th percentile.")
            return False
        ...
        logger.info(
            f"Added {len(name2demo)} demos (one each) across all predictors.")
        return True
