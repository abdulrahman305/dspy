...
class DspyGEPAResult:
    ...
    def highest_score_achieved_per_val_task(self) -> list[float]:
        return [
            self.val_subscores[list(self.per_val_instance_best_candidates[val_idx])[
                0]][val_idx]
    ...
    def from_gepa_result(gepa_result: "GEPAResult", adapter: "DspyAdapter") -> "DspyGEPAResult":
        return DspyGEPAResult(
            candidates=[adapter.build_program(c)
        ...
        *,
        ...
        if self.auto is not None:
            self.max_metric_calls = self.auto_budget(
                ...
                valset_size=len(
                    valset) if valset is not None else len(trainset),
            )
        ...
        feedback_map = {k: feedback_fn_creator(
            k, v) for k, v in student.named_predictors()}
        ...
        base_program = {name: pred.signature.instructions for name,
                        pred in student.named_predictors()}
        gepa_result: GEPAResult = optimize(
            ...
            reflection_lm=((lambda x: self.reflection_lm(
                x)[0]) if self.reflection_lm is not None else None),
