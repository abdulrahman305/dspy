...
class COPRO(Teleprompter):
    ...
    def _check_candidates_equal(self, candidate1, candidate2):
        for p1, p2 in zip(
            ...
            if self._get_signature(p1).instructions != self._get_signature(
                    p2).instructions:
                return False
    ...
    def compile(self, student, *, trainset, eval_kwargs):
        ...
        for predictor in module.predictors():
            ...
            *_, last_key = self._get_signature(predictor).fields.keys()
            ...
            basic_prefix = self._get_signature(
                predictor).fields[last_key].json_schema_extra["prefix"]
        ...
        for d in range(
            ...
            for p_i, (p_old, p_new) in enumerate(
                    zip(module.predictors(),
                        module_clone.predictors(),
                        strict=False)):
                candidates_ = latest_candidates[id(
                    p_old
                ...
                for c_i, c in enumerate(candidates_):
                    ...
                    *_, last_key = self._get_signature(p_new).fields.keys()
                    updated_signature = (
                        self._get_signature(p_new).with_instructions(
                            instruction).with_updated_fields(last_key,
                                                             prefix=prefix))
                    ...
                    replace_entry = True
                    logger.debug(
                        f"(instruction, prefix) {(instruction, prefix)}")
                    if (instruction,
                            prefix) in evaluated_candidates[id(p_old)]:
                        if evaluated_candidates[id(p_old)][(
                                instruction, prefix)]["score"] >= score:
                            replace_entry = False

                    if replace_entry:
                        # Add it to our evaluated candidates list
                        evaluated_candidates[id(p_old)][(
                            instruction, prefix)] = {
                                "score": score,
                ...
                if self.track_stats:
                    ...
                    results_latest[id(p_old)]["std"].append(
                        np.std(latest_scores))
                ...
                *_, last_key = self._get_signature(p_old).fields.keys()
                updated_signature = (
                    self._get_signature(p_new).with_instructions(
                        best_candidate["instruction"]).with_updated_fields(
                            last_key, prefix=best_candidate["prefix"]))
                self._set_signature(p_new, updated_signature)
            ...
            for p_base in module.predictors():
                ...
                shortest_len = min(len(evaluated_candidates[id(p_base)]),
                                   shortest_len)
                ...
                all_candidates[id(
                    p_base)].proposed_prefix_for_output_field.extend(
        ...
        for predictor in module.predictors():
            candidates.extend(
                list(evaluated_candidates[id(predictor)].values()))

            if self.track_stats:
                best_predictors = list(
                    evaluated_candidates[id(predictor)].values())
                best_predictors.sort(key=lambda x: x["score"], reverse=True)

                scores = [x["score"] for x in best_predictors][:10]
                results_best[id(predictor)]["depth"].append(d)
                results_best[id(predictor)]["max"].append(max(scores))
                results_best[id(predictor)]["average"].append(
                    sum(scores) / len(scores))
