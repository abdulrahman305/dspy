...
class SignatureMeta(type(BaseModel)):
    ...
    def __new__(mcs, signature_name, bases, namespace, **kwargs):
        ...
        ordered_annotations = {
            ...
            for name in field_order if name in raw_annotations
        }
        # Add any remaining annotations that weren't in field_order
        ordered_annotations.update({
            k: v
            for k, v in raw_annotations.items() if k not in ordered_annotations
        })
...
class Signature(BaseModel, metaclass=SignatureMeta):
    ...
    @classmethod
    def with_updated_fields(cls,
                            name: str,
                            type_: type | None = None,
                            **kwargs: dict[str, Any]) -> type["Signature"]:
    ...
    def insert(cls,
               ...
               name: str,
               field,
               type_: type | None = None) -> type["Signature"]:
        ...
        lst = input_fields if field.json_schema_extra[
            "__dspy_field_type"] == "input" else output_fields
    ...
    def equals(cls, other) -> bool:
        ...
        for name in cls.fields.keys() | other.fields.keys():
            if name not in other.fields or name not in cls.fields:
                return False
            if cls.fields[name].json_schema_extra != other.fields[
    ...
    def dump_state(cls):
        ...
        for field in cls.fields:
            state["fields"].append({
                "prefix":
                cls.fields[field].json_schema_extra["prefix"],
                "description":
                cls.fields[field].json_schema_extra["desc"],
            })
    ...
    def load_state(cls, state):
        ...
        for field, saved_field in zip(signature_copy.fields.values(),
                                      state["fields"],
                                      strict=False):
...
def ensure_signature(signature: str | type[Signature],
                     instructions=None) -> type[Signature]:
...
def _parse_signature(signature: str,
                     names=None) -> dict[str, tuple[type, Field]]:
...
def _parse_field_string(field_string: str, names=None) -> dict[str, str]:
    ...
    types = [
        str if arg.annotation is None else _parse_type_node(
...
def _parse_type_node(node, names=None) -> Any:
    ...
    if isinstance(node, ast.Subscript):
        ...
        if isinstance(slice_node, ast.Tuple):
            arg_types = tuple(
                _parse_type_node(elt, names) for elt in slice_node.elts)
        else:
    ...
    if isinstance(node, ast.Call) and isinstance(
            node.func, ast.Name) and node.func.id == "Field":
