...
class GenerateModuleInstruction(dspy.Module):
    ...
    def forward(
                ...
                for example in candidate_set:
                    if "augmented" in example.keys():
                        fields_to_use = get_signature(
                            program.predictors()[pred_i]).fields
        ...
        basic_instruction = get_signature(
            program.predictors()[pred_i]).instructions
        task_demos = ""

        if self.use_task_demos:
            # Combine current and adjacent sets
            adjacent_sets = ([demo_candidates[pred_i][demo_set_i]] +
        ...
        if self.program_aware:
            try:
                ...
                for field_name, field in get_signature(
                        program.predictors()[pred_i]).fields.items():
