...
class ColBERTv2:
    ...
    def __call__(
        ...
        if self.post_requests:
            topk: list[dict[str,
                            Any]] = colbertv2_post_request(self.url, query, k)
        else:
            topk: list[dict[str,
                            Any]] = colbertv2_get_request(self.url, query, k)
...
class ColBERTv2RetrieverLocal:
    ...
    def build_index(self):
        ...
        with Run().context(
            ...
            indexer = Indexer(checkpoint=self.colbert_config.checkpoint,
    ...
    def get_index(self):
        ...
        with Run().context(
                RunConfig(experiment=self.colbert_config.experiment)):
            searcher = Searcher(index=self.colbert_config.index_name,
                                collection=self.passages)
        return searcher
...
class ColBERTv2RerankerLocal:
    ...
    def forward(self, query: str, passages: list[str] | None = None):
        ...
        doc_ids, doc_masks = col.doc(doc_ids,
                                     doc_masks,
