...
class Cache:
    ...
    def cache_key(
        ...
        params = {
            k: transform_value(v)
            for k, v in request.items() if k not in ignored_args_for_cache_key
        }
    ...
    def get(
        ...
        try:
            ...
            logger.debug(
                f"Failed to generate cache key for request: {request}")
            return None
    ...
    def put(
        ...
        if self.enable_disk_cache:
            try:
                ...
                logger.debug(
                    f"Failed to put value in disk cache: {value}, {e}")
