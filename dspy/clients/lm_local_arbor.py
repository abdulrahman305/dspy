...
class ArborReinforceJob(ReinforceJob):
    ...
    def _run_grpo_step_one_group(
        ...
        url = urljoin(
            api_base, f"fine_tuning/grpo/{self.provider_job_id}/step")
        headers = {"Content-Type": "application/json"}
    ...
    def save_checkpoint(self, checkpoint_name: str, score: float | None = None):
        api_base = self.lm.kwargs["api_base"]
        url = urljoin(
            api_base, f"fine_tuning/grpo/{self.provider_job_id}/checkpoint")
        headers = {"Content-Type": "application/json"}
    ...
    def terminate(self):
        ...
        url = urljoin(
            api_base, f"fine_tuning/grpo/{self.provider_job_id}/terminate")
        headers = {"Content-Type": "application/json"}
        body = {"job_id": self.provider_job_id}
        response = requests.post(url, headers=headers, json=body)
    ...
    def cancel(self):
        if self.provider_job_id:
            api_base = self.lm.kwargs["api_base"]
            url = urljoin(
                api_base, f"fine_tuning/grpo/{self.provider_job_id}/cancel")
            headers = {"Content-Type": "application/json"}
