...
class ArborReinforceJob(ReinforceJob):
    ...
    def initialize(self):
        ...
        lr_scheduler_type = self.train_kwargs.get(
            "lr_scheduler_type",
            self.DEFAULT_TRAIN_KWARGS["lr_scheduler_type"])
        max_prompt_length = self.train_kwargs.get(
            "max_prompt_length",
            self.DEFAULT_TRAIN_KWARGS["max_prompt_length"])
        max_completion_length = self.train_kwargs.get(
            "max_completion_length",
            self.DEFAULT_TRAIN_KWARGS["max_completion_length"])
        ...
        max_context_length = self.train_kwargs.get(
            "max_context_length",
            self.DEFAULT_TRAIN_KWARGS["max_context_length"])
...
class ArborProvider(Provider):
    ...
    def kill(lm: "LM", launch_kwargs: dict[str, Any] | None = None):
        ...
        response = requests.post(f"{api_base}chat/kill", )
    ...
    def get_training_status(job_id: str,
                            training_kwargs: dict[str, Any]) -> TrainingStatus:
    ...
    @staticmethod
    def _start_remote_training(train_file_id: str, model: str,
                               train_kwargs: dict[str, Any]) -> str:
